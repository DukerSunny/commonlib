...
public class StatusBarUtil {
    ...
    /**
     * 1.不是图片渗透
     *   如果是6.0以上→设置状态栏字体颜色黑色，设置状态栏颜色
     *   如果是6.0以下，4.4以上→状态栏字体颜色默认白色，设置状态栏颜色（白色变黑色，其他不变）
     *   如果是4.4以下→return
     * 2.渗透图片
     *   如果是6.0以上→设置状态栏字体颜色黑色，设置渗透图
     *   如果是4.4以上，6.0以下→状态栏字体颜色默认，设置渗透图
     *   如果是4.4以下→return（可能需要修改某布局）
     */
    /**
     * 第一种
     *
     ...
     * @param bgColor
     */
    public static void setStatusBarFontAndBack(Activity activity, int bgColor) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            //6.0以上
//            setStatusBarFontDarkColor(activity);
            // 设置黑字体
            activity.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
            setColor(activity, bgColor, 0); //背景色
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            //4.4以上
            if (bgColor == Color.WHITE) {
                //如果背景是白色，则设置为黑色
                setColor(activity, Color.BLACK, 0); //背景色
                return;
            }
            setColor(activity, bgColor, 0); //背景色
            setStatusBarFontDarkColor(activity);    //设置黑字体
        }
    }
     ...
     * @param activity
     */
    public static void setStatusBarImg(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            transparentStatusBar(activity);
            StatusBarLightMode(activity);    //6.0生效，设置黑字体
        }
    }
...
//            setRootView(activity);    //4.4手机问题，获取content为空。布局设置fitsystemwindows
        }
    }
